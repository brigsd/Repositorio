...
export const A2_REGISTROS: UnidadeCurriculo = {
  ...
  ancoraPropósito: {
    ...
    corpo: `Você já sabe se comunicar. Todo dia você manda mensagem, resolve situação pelo celular e explica o que precisa sem dificuldade. Essa habilidade já é sua, e não tem nada de errado com ela.\n\nO que essa unidade traz é uma segunda chave. Cada situação tem um tom certo: a mensagem para o amigo é diferente da mensagem para o chefe, que é diferente de um e-mail para um cliente ou de um requerimento numa repartição. Nenhuma dessas formas está errada — elas são registros diferentes, ferramentas para contextos diferentes.\n\nO problema acontece quando a gente usa a chave errada na hora errada. Uma mensagem informal no momento profissional pode fechar uma porta que você nem sabe que estava aberta.`,
