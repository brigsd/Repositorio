...
**Base pedagógica:** Kintsch & van Dijk 1978; Fuzzy Trace Theory; Keys to Literacy/Shanahan (ideia principal é habilidade central, mas nenhuma fonte atribui percentual específico). Princípio 4 (GIST procedure / Cunningham 1982) recomenda produção > identificação. DECISÃO: `escrita_lacuna` usa comparação exata de string — inadequado para GIST de ideia principal (aluno correto semanticamente levaria erro). GIST real exigiria `ExercicioReescrita` (AI-avaliado, como na A.2) — não implementado na B.2 por ora. Combinação 16 exercícios de identificação + projetoIntegrador (produção livre) mantida como design adequado para o público EJA.
